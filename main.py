...
def video() :
    ...
    flash('У вас будет 45 секунд на обсуждение указанной выше темы. Из-за ограничений мы не можем перенаправить вас, когда видео закончится. Переместите URL-адрес в / video_dash вместо / video_1 еще раз. Тогда вы сможете увидеть свои результаты.')
...
def audio_index():
    ...
    # Flash message
    flash("После нажатия кнопки выше у вас будет 15 секунд, чтобы ответить на вопрос.")
...
def audio_recording():
    ...
    # Send Flash message
    flash("Запись окончена! Теперь у вас есть возможность провести анализ своих эмоций. При желании вы также можете записать себя снова.")
...
def text_1():
    ...
    traits = ['Экстраверсия', 'Невротизм', 'Доброжелательность', 'Добросовестность', 'Открытость']
    probas = get_personality(text)[0].tolist()
    print(get_personality(text))
    ...
    return render_template('text_dash.html', traits = probas, trait = trait, num_words = num_words, common_words = common_words_perso)
